...
export default function NewsForm({ initialData }: { initialData?: NewsArticle }) {
  ...
  const [form, setForm] = useState<Partial<NewsArticle>>({
    title: '', content: '', category: '行业知识', img: '', keywords: [], seoTitle: '', seoDescription: '', seoSlug: ''
  });
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-10 animate-in slide-in-from-bottom-8 duration-700 pb-20">
        <div className="flex justify-between items-center px-6">
          <div>
             <h2 className="text-5xl font-black uppercase tracking-tighter text-slate-900 leading-none">{initialData ? '编辑文章' : '撰写新文章'}</h2>
             <p className="text-slate-400 font-bold uppercase tracking-widest text-[9px] mt-2 tracking-[0.3em] flex items-center gap-2">
                <span className="w-1.5 h-1.5 rounded-full bg-blue-600"></span> 品牌内容生产与 Google SEO 流量中心
             </p>
          </div>
          <div className="flex gap-4">
             ...
             <button onClick={handleSubmit} disabled={loading} className="bg-blue-600 text-white px-12 py-5 rounded-[24px] font-black uppercase tracking-[0.2em] text-xs shadow-2xl shadow-blue-500/20 hover:bg-slate-900 transition-all active:scale-95 disabled:opacity-50">
                {loading ? '发布中...' : (initialData ? '保存修改' : '立即发布')}
             </button>
          </div>
       </div>
        ...
        <div className="lg:col-span-8 space-y-10">
            <div className="bg-white p-12 rounded-[56px] shadow-sm border border-slate-100">
               <h3 className="text-[10px] font-black uppercase text-blue-600 tracking-[0.3em] mb-12 flex items-center gap-4">
                  <div className="w-10 h-10 rounded-2xl bg-blue-50 flex items-center justify-center text-xs"><i className="fas fa-pen-nib"></i></div> 核心内容编辑
               </h3>
               <div className="space-y-10">
                 <div>
                   <label className="block text-[9px] font-black uppercase text-slate-400 mb-3 tracking-[0.3em] ml-1">文章大标题 * (英文)</label>
                   <input 
                     type="text" 
                     value={form.title}
                     onChange={(e) => {
                       ...
                       if (!form.seoTitle || form.seoTitle === `${form.title} | Factory News & Industry Insights`) {
                         updates.seoTitle = `${title} | Fastener Industry Insights`;
                       }
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                     <div>
                        <label className="block text-[9px] font-black uppercase text-slate-400 mb-3 tracking-[0.3em] ml-1">文章分类</label>
                        <select 
                          value={form.category}
                        ...
                        >
                          <option value="行业知识">行业知识 (Industry Knowledge)</option>
                          <option value="公司动态">公司动态 (Company Update)</option>
                          <option value="展会信息">展会信息 (Exhibition)</option>
                          <option value="技术指南">技术指南 (Technical Guide)</option>
                        </select>
                     </div>
                     <div>
                        <label className="block text-[9px] font-black uppercase text-slate-400 mb-3 tracking-[0.3em] ml-1">SEO URL 路径 (Slug)</label>
                        <input 
                          type="text" 
                          value={form.seoSlug}
                 ...
                 <div className="bg-slate-900 rounded-[40px] p-10 text-white overflow-hidden shadow-2xl relative border-4 border-slate-800">
                     ...
                     <h4 className="text-[9px] font-black uppercase text-blue-400 tracking-[0.4em] mb-8 relative z-10 flex items-center gap-2"><i className="fab fa-google"></i> Google 搜索结果预览 (实时)</h4>
                     <div className="bg-white rounded-[24px] p-8 relative z-10 text-slate-900 border border-slate-200">
                         ...
                         <h4 className="text-[#1a0dab] text-xl font-medium mb-1 hover:underline leading-tight">{form.seoTitle || form.title || '文章标题'}</h4>
                         <p className="text-[#4d5156] text-[13px] leading-relaxed line-clamp-2">{form.seoDescription || '在这里预览 Google 搜索结果中的网页描述，建议包含关键词以提高点击率...'}</p>
                     </div>
                 </div>
                 
                 <div>
                    <label className="block text-[9px] font-black uppercase text-slate-400 mb-3 tracking-[0.3em] ml-1">正文详情 (支持富文本与 HTML) *</label>
                    <div className="border-4 border-slate-50 rounded-[40px] overflow-hidden focus-within:border-blue-100 transition shadow-inner">
                       <TiptapEditor 
                         ...
                         onOpenLibrary={() => setShowMatPicker(true)}
         ...
         <div className="lg:col-span-4 space-y-10">
             <div className="bg-white p-12 rounded-[64px] shadow-sm border border-slate-100 sticky top-32">
                 <h3 className="text-[10px] font-black uppercase text-blue-600 tracking-[0.3em] mb-12 flex items-center gap-4">
                     <div className="w-10 h-10 rounded-2xl bg-blue-50 flex items-center justify-center"><i className="fas fa-image"></i></div> 文章封面图
                 </h3>
                 ...
                 >
                     {form.img ? (
                         ...
                         <div className="absolute inset-0 bg-slate-900/60 opacity-0 group-hover:opacity-100 transition flex items-center justify-center">
                             <div className="bg-white px-8 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-2xl">更换封面</div>
                         </div>
                         </>
                     ) : (
                         <div className="text-center">
                             <i className="fas fa-camera text-4xl text-slate-200 mb-4 group-hover:text-blue-200 transition"></i>
                             <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">从素材库选择</p>
                         </div>
                     )}
                 </div>
                 ...
                 <div className="mt-12 space-y-8">
                    <div>
                       <label className="block text-[9px] font-black uppercase text-slate-400 mb-3 tracking-[0.3em] ml-1">SEO Title (网页标题)</label>
                    ...
                    <div>
                       <label className="block text-[9px] font-black uppercase text-slate-400 mb-3 tracking-[0.3em] ml-1">SEO Description (网页描述)</label>
          ...
          <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-xl z-[200] flex items-center justify-center p-6 md:p-12">
             <div className="bg-white w-full max-w-6xl rounded-[80px] shadow-2xl h-full max-h-[900px] flex flex-col overflow-hidden animate-in zoom-in duration-700 relative">
                <div className="p-12 border-b border-slate-100 flex justify-between items-center relative z-10">
                   <div>
                     ...
                     <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] mt-3 flex items-center gap-2">
                        <span className="w-2 h-2 rounded-full bg-blue-600"></span> 请选择文章插图或封面
                     </p>
                   </div>
                ...
                <div className="p-12 border-t border-slate-100 flex justify-end items-center bg-white relative z-10">
                   <button onClick={() => setShowMatPicker(false)} className="bg-slate-900 text-white px-16 py-6 rounded-[32px] font-black uppercase tracking-[0.4em] text-xs shadow-2xl shadow-slate-300 hover:bg-blue-600 transition-all active:scale-95">确定选择并关闭</button>
                </div>
             </div>
          </div>
      )}
