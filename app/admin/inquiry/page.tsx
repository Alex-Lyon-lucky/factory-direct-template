  ...
  return (
    <div className="max-w-6xl mx-auto space-y-12 animate-in fade-in duration-700 pb-20">
      <div className="flex justify-between items-end px-4">
        <div>
           <h2 className="text-4xl font-black italic uppercase tracking-tighter text-slate-900 leading-none mb-2">客户询盘中心</h2>
           <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px] mt-2 italic">Global RFQ & Business Inquiries Inbox</p>
        </div>
        <div className="bg-white px-8 py-4 rounded-3xl border border-slate-100 shadow-sm flex items-center gap-6">
           <div className="text-center">
              ...
              <div className="text-xl font-black italic text-slate-900">{inquiries.length}</div>
           </div>
           <div className="w-px h-8 bg-slate-100"></div>
           <div className="text-center">
              <div className="text-[10px] font-black text-red-400 uppercase tracking-widest mb-1">待处理</div>
              <div className="text-xl font-black italic text-red-500">{inquiries.filter(i => i.status === 'New').length}</div>
           </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        {inquiries.map(inq => (
          ...
          >
            <div className={`w-20 h-20 rounded-3xl flex items-center justify-center text-3xl font-black italic shadow-xl transition-all ${inq.status === 'New' ? 'bg-blue-600 text-white shadow-blue-200' : 'bg-slate-50 text-slate-300'}`}>
               {inq.name.charAt(0)}
            </div>
            <div className="flex-1">
               <div className="flex items-center gap-4 mb-2">
                  <h3 className="text-2xl font-black italic uppercase tracking-tighter text-slate-900">{inq.name}</h3>
        ...
        {inquiries.length === 0 && (
          <div className="text-center py-32 bg-white rounded-[64px] border-2 border-dashed border-slate-100 text-slate-300 font-black uppercase italic tracking-widest">
            ...
            <div className="bg-white h-full w-full max-w-2xl rounded-[64px] shadow-2xl flex flex-col animate-in slide-in-from-right-full duration-700 relative overflow-hidden">
               ...
               <div className="p-12 border-b border-slate-50 flex justify-between items-center relative z-10">
                  ...
                  <div className="flex gap-4">
                     ...
                     <a href={`mailto:${selectedInq.email}`} className="bg-blue-600 text-white px-10 rounded-2xl font-black uppercase italic text-xs tracking-widest flex items-center gap-3 shadow-xl shadow-blue-200 hover:bg-slate-900 transition">
               ...
               <div className="flex-1 p-16 overflow-y-auto no-scrollbar relative z-10">
                  <div className="mb-16">
                     ...
                     <h2 className="text-5xl font-black italic uppercase tracking-tighter text-slate-900 mb-6">{selectedInq.name}</h2>
                     <div className="space-y-4">
                        <div className="flex items-center gap-4 text-slate-600 font-bold italic"><i className="fas fa-envelope text-blue-500 w-6"></i> {selectedInq.email}</div>
                        {selectedInq.phone && <div className="flex items-center gap-4 text-slate-600 font-bold italic"><i className="fab fa-whatsapp text-emerald-500 w-6"></i> {selectedInq.phone}</div>}
                        {selectedInq.company && <div className="flex items-center gap-4 text-slate-600 font-bold italic"><i className="fas fa-building text-slate-400 w-6"></i> {selectedInq.company}</div>}
                        <div className="flex items-center gap-4 text-slate-400 font-bold italic text-sm"><i className="far fa-clock text-slate-300 w-6"></i> Received: {new Date(selectedInq.date).toLocaleString()}</div>
                     </div>
                  </div>
                  ...
                  <div className="space-y-6">
                     ...
                     <div className="bg-slate-50 p-12 rounded-[48px] border border-slate-100 relative shadow-inner">
                        ...
                        <p className="text-xl font-bold text-slate-700 leading-loose italic relative z-10">
                           {selectedInq.message}
                        </p>
                     </div>
                  </div>
