...
export default function AdminDashboard() {
  ...
  const stats = [
    ...
    { label: 'Google SEO 优化率', value: '98.5%', icon: 'fas fa-chart-line', color: 'bg-emerald-500', href: '#' },
  ];
    ...
    <div className="max-w-7xl mx-auto space-y-12 animate-in fade-in duration-700 pb-20 px-4">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
           ...
           <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px] mt-2 tracking-widest">Hangfan Industrial OS Dashboard v3.0</p>
        </div>
        
        {/* Google SEO Real-time Engine Display */}
        <div className="bg-white px-8 py-5 rounded-[32px] border border-slate-100 shadow-sm flex items-center gap-6 min-w-[320px]">
           <div className="w-12 h-12 bg-emerald-50 rounded-2xl flex items-center justify-center text-emerald-600">
              <i className="fas fa-shield-check text-xl"></i>
           </div>
           <div className="flex-1">
              <div className="flex justify-between items-center mb-2">
                 <span className="text-[10px] font-black uppercase tracking-widest text-slate-900">Google SEO Indexing</span>
                 <span className="text-[10px] font-black text-emerald-600">HEALTHY</span>
              </div>
              <div className="w-full h-1.5 bg-slate-50 rounded-full overflow-hidden">
                 <div className="bg-emerald-500 h-full w-[98%] shadow-lg shadow-emerald-200 animate-pulse"></div>
              </div>
           </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white p-12 rounded-[56px] shadow-sm border border-slate-100 flex flex-col justify-between h-[520px]">
           <div className="flex justify-between items-center mb-10 px-2">
              <div>
                <h3 className="text-xl font-black uppercase tracking-tighter text-slate-900">数据分析快报</h3>
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-2">Traffic Intelligence Summary</p>
              </div>
              <Link href="/admin/analytics" className="text-[10px] font-black uppercase text-blue-600 tracking-widest hover:underline bg-blue-50 px-5 py-3 rounded-2xl">进入全屏中心 &rarr;</Link>
           </div>
           ...
           <div className="grid grid-cols-3 gap-8 mb-10 px-2">
              <div className="p-8 bg-slate-50 rounded-[32px] group hover:bg-white hover:shadow-xl transition-all cursor-default border border-slate-50 hover:border-blue-100">
                 <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3">24h Visitors</p>
                 <div className="flex items-end gap-2">
                   <p className="text-3xl font-black text-slate-900 tracking-tighter leading-none">44</p>
                   <span className="text-[10px] font-black text-emerald-500 pb-1">+4%</span>
                 </div>
              </div>
              <div className="p-8 bg-slate-50 rounded-[32px] group hover:bg-white hover:shadow-xl transition-all cursor-default border border-slate-50 hover:border-blue-100">
                 <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3">Sessions</p>
                 <div className="flex items-end gap-2">
                   <p className="text-3xl font-black text-slate-900 tracking-tighter leading-none">54</p>
                   <span className="text-[10px] font-black text-emerald-500 pb-1">+12%</span>
                 </div>
              </div>
              <div className="p-8 bg-slate-50 rounded-[32px] group hover:bg-white hover:shadow-xl transition-all cursor-default border border-slate-50 hover:border-blue-100">
                 <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3">Avg. Stay</p>
                 <div className="flex items-end gap-2">
                   <p className="text-3xl font-black text-slate-900 tracking-tighter leading-none">2s</p>
                   <span className="text-[10px] font-black text-red-400 pb-1">-3s</span>
                 </div>
              </div>
           </div>
           ...
           <div className="flex-1 flex items-end gap-3 px-6 h-32">
             {[30, 50, 40, 80, 60, 20, 70, 40, 60, 50, 90, 30, 50, 70, 40, 25, 85, 45, 65, 35].map((h, i) => (
               <div key={i} className="flex-1 bg-blue-600/10 rounded-t-lg hover:bg-blue-600 transition-all duration-500" style={{ height: `${h}%` }}></div>
             ))}
           </div>
              ...
              {inquiries.slice(0, 6).map((inq) => (
                 <div key={inq.id} className="p-6 bg-white/5 rounded-[32px] border border-white/5 hover:bg-white/10 transition-all group cursor-pointer">
                    <div className="flex justify-between items-start mb-3">
                       ...
                       <span className="text-[8px] font-bold text-slate-500 uppercase tracking-widest">{inq.createdAt ? new Date(inq.createdAt).toLocaleDateString() : 'Now'}</span>
                    </div>
